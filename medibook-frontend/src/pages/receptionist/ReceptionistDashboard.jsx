...
import { CalendarRange, CalendarX2, CalendarClock } from 'lucide-react'
...
export default function ReceptionistDashboard() {
  ...
  const { data: rescheduleSlots, refetch: refetchRescheduleSlots } = useApi(
    () => appointmentService.getAvailableSlots({ doctorId: rescheduleTarget?.doctorId, date: rescheduleDate }),
    [rescheduleTarget, rescheduleDate],
    !!rescheduleTarget,
  )

  const appointments = (Array.isArray(data) ? data : []).sort((a, b) => new Date(a.startTime) - new Date(b.startTime))

  const loadByDoctor = (Array.isArray(doctors) ? doctors : []).map((d) => ({
  ...
  const handleReschedule = async () => {
    ...
    try {
      await appointmentService.rescheduleAppointment(rescheduleTarget.id, {
        newSlotId: selectedRescheduleSlot.id,
        isEmergency: rescheduleEmergency,
        emergencyReason: rescheduleEmergency ? rescheduleReason.trim() : undefined,
      })
      showToast(`Appointment moved to ${formatTime(selectedRescheduleSlot.startTime)}.`, 'success')
      setRescheduleTarget(null)
      setSelectedRescheduleSlot(null)
      setRescheduleEmergency(false)
      setRescheduleReason('')
      refetch()
      refetchRescheduleSlots()
    } catch (err) {
      showToast(err.message || 'Could not reschedule this appointment.', 'error')
    } finally {
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button variant="primary" isLoading={isRescheduling} onClick={handleReschedule} disabled={!selectedRescheduleSlot}>
              Confirm move
            </Button>
          </>
        }
      >
        {rescheduleTarget && (
          <div className="flex flex-col gap-4">
            <p className="text-sm text-ink-muted">
              Moving <span className="font-medium text-ink">{rescheduleTarget.patientName}</span> from{' '}
              <span className="font-medium text-ink">{formatTime(rescheduleTarget.startTime)}</span> with{' '}
              <span className="font-medium text-ink">{rescheduleTarget.doctorName}</span>.
            </p>

            <Field label="New date">
              <Input type="date" min={todayISO()} value={rescheduleDate} onChange={(e) => {
            ...
            <label className="flex items-center gap-2 text-sm text-ink-muted">
              <input
                ...
                onChange={(e) => {
                  setRescheduleEmergency(e.target.checked)
                  if (!e.target.checked) setRescheduleReason('')
                }}
              />
              Emergency reschedule (overrides the 2-hour rule)
            </label>

            {rescheduleEmergency && (
              <div>
                <label className="block text-sm font-medium text-ink mb-1.5">Emergency reason</label>
                <input
                  type="text"
                  value={rescheduleReason}
                  onChange={(e) => setRescheduleReason(e.target.value)}
                  placeholder="Explain why this move is urgent"
            ...
            <div>
              <p className="mb-2 text-sm font-medium text-ink">Choose a replacement slot</p>
              {rescheduleSlots && rescheduleSlots.length === 0 && (
                <EmptyState icon={CalendarRange} title="No open slots" description="Try another date for this doctor." />
              )}
              {rescheduleSlots && rescheduleSlots.length > 0 && (
                  ...
                  {rescheduleSlots.map((slot) => (
                    <button
                      ...
                      className={`rounded-card border px-3 py-2 text-sm ${
                        selectedRescheduleSlot?.id === slot.id ? 'border-primary-600 bg-primary-50 text-primary-700' : 'border-surface-border bg-white text-ink'
                      }`}
                    >
